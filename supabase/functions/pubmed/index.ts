...
async function handler(req: Request): Promise<Response> {
  ...
  try {
    ...
    // Validate input
    if (!title || !abstract) {
      return new Response(JSON.stringify({ error: 'Title and abstract are required' }), {
        status: 400,
...
`;
    ...
    const response = await ai.models.generateContent({
      ...
      config: {
        ...
        responseSchema: {
            ...
            required: ["translatedTitle", "translatedAbstract"],
        },
      }
    });
    ...
    return new Response(JSON.stringify({ error: 'Failed to translate content.', details: errorMessage }), {
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      status: 500,
    });
  }
